...
import {
  ...
  ApiOutlined, RocketOutlined, BarChartOutlined
...
export default function TestExecution() {
  ...
  useEffect(() => {
    ...
    // 监听接口添加事件，自动刷新接口列表
    const unsubscribe = eventBus.on('apiAdded', loadApis)
    return unsubscribe
  }, [])
  ...
  const handleExecute = async () => {
    ...
    try {
      const response = await testApis.execute({
        ...
        requestInterval: values.requestInterval || 0,
        ...
        <Col span={10}>
          <Card title={<span style={{ fontWeight: 600 }}>测试配置</span>}
            ...
            <Form form={form} layout="vertical" initialValues={{
              concurrency: 1, timeout: 30, retries: 3, retryInterval: 1, requestInterval: 0,
            }}>
              ...
              <Row gutter={12}>
                ...
                <Col span={12}>
                  <Form.Item name="requestInterval" label="请求间隔(ms)"
                    tooltip="每两个请求之间的等待时间，0 表示无间隔">
              ...
              <Form.Item name="retryInterval" label="重试间隔(秒)" style={{ display: 'none' }}>
                <InputNumber />
              </Form.Item>
          ...
          {!progress && !running && !result && !error && (
            <div style={{
              ...
              <div style={{ fontSize: 13, color: '#9ca3af', marginBottom: 32 }}>
                选择左侧的接口和数据集，点击「开始测试」即可执行
              </div>
              <Row gutter={16} style={{ width: '100%', maxWidth: 400 }}>
                <Col span={8}>
                  <div style={{ textAlign: 'center', padding: '16px 0' }}>
                    <ApiOutlined style={{ fontSize: 24, color: '#6366f1', marginBottom: 8 }} />
                    <div style={{ fontSize: 12, color: '#6b7280', fontWeight: 500 }}>选择接口</div>
                  </div>
                </Col>
                <Col span={8}>
                  <div style={{ textAlign: 'center', padding: '16px 0' }}>
                    <DatabaseOutlined style={{ fontSize: 24, color: '#8b5cf6', marginBottom: 8 }} />
                    <div style={{ fontSize: 12, color: '#6b7280', fontWeight: 500 }}>选择数据集</div>
                  </div>
                </Col>
                <Col span={8}>
                  <div style={{ textAlign: 'center', padding: '16px 0' }}>
                    <BarChartOutlined style={{ fontSize: 24, color: '#a78bfa', marginBottom: 8 }} />
                    <div style={{ fontSize: 12, color: '#6b7280', fontWeight: 500 }}>查看报告</div>
                  </div>
                </Col>
              </Row>
            </div>
